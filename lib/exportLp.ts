...
/**
 * 生成LPを ES モジュール形式の JS 文字列にする。
 * - `lpHtml` エクスポート: HTML文字列
 * - `renderLP(container)`: DOM要素にHTMLを注入するユーティリティ
 * エンジニア連携や外部CMSへの埋め込み用途を想定。
 */
export function buildLpJs(
  categories: SectionCategory[],
  selected: SelectedSections,
  origin: string,
  theme?: ThemeSelection,
  ...
  return `/**
 * Generated LP — JavaScript ES Module
 * Generated by Marketer's Studio
 *
 * Usage:
 *   import { renderLP } from './generated-lp.js';
 *   renderLP(document.getElementById('lp-root'));
 *
 * Or with plain <script type="module">:
 *   import { renderLP } from './generated-lp.js';
 *   renderLP(document.body);
 */

export const lpHtml = \`${escaped}\`;

/**
 * Inject the LP into a DOM element.
 * @param {HTMLElement} container - defaults to document.body
 */
export function renderLP(container = document.body) {
  container.innerHTML = lpHtml;
}

export default lpHtml;
`;
}
